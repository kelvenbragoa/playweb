...
let dataIdBeingDeleted = ref(null);
 ...
 const getData = async (page = 1) => {
  axios.get(`/owner-recharges?page=${page}`,
      {
        params:{
...
axios.delete(`/owner-recharges/${dataIdBeingDeleted}`)
.then((response)=>{
                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-header">
                                        ...
                                        <router-link to="/owner/recharges/create" class="btn btn-pill btn-primary mt-3"><vue-feather type="plus"></vue-feather>Adicionar</router-link> 
                                            ...
                                            <table class="table table-striped">
                                                ...
                                                <tbody v-if="retriviedData.data.length > 0">
                                                    <tr  v-for="(actualData,index) in retriviedData.data" :key="actualData.id">
                                                        <td>#{{ index + 1 }}</td>
                                                        <td>{{ actualData.user.name +' ' +actualData.user.surname+' /' +actualData.user.email+'/ ' +actualData.user.mobile}}</td>
                                                        <td>{{ actualData.price}}MT</td>
                                                        <td>{{ actualData.ref}}</td>
                                                        <td>{{ actualData.method}}</td>
                                                        <td>
                                                            <!-- <router-link :to="'/owner/recharges/'+actualData.id+'/edit'"><vue-feather type="edit-2"></vue-feather></router-link> -->
                                                            <router-link :to="'/owner/recharges/'+actualData.id"><vue-feather type="eye"></vue-feather></router-link> 
